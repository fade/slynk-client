...
are communications problems."
  (destructure-case event
    ...
    ((:channel-send id msg)
     (print (list :channel-send id msg) *error-output*))
    ((:emacs-channel-send id msg)
     (slime-send `(:emacs-channel-send ,id ,msg) connection))
    ((:read-from-minibuffer thread tag prompt initial-value)
     (print (list :read-from-minibuffer thread tag prompt initial-value) *error-output*))
    ((:y-or-n-p thread tag question)
     (print (list :y-or-n-p thread tag question) *error-output*))
    ...
    ((:indentation-update info)
     (declare (ignore info)))
    ((:eval-no-wait form)
     (print (list :eval-no-wait form) *error-output*))
    ((:eval thread tag form-string)
     (print (list :eval thread tag form-string) *error-output*))
    ((:ed-rpc-no-wait function-name &rest args)
     (print (list :ed-rpc-no-wait function-name '&rest args) *error-output*))
    ((:ed-rpc thread tag function-name &rest args)
     (print (list :ed-rpc thread tag function-name '&rest args) *error-output*))
    ((:emacs-return thread tag value)
     (slime-send `(:emacs-return ,thread ,tag ,value) connection))
    ((:ed what)
     (print (list :ed what) *error-output*))
    ((:inspect what wait-thread wait-tag)
     (print (list :inspect what wait-thread wait-tag) *error-output*))
    ((:background-message message)
     (print (list :background-message message) *error-output*))
    ((:debug-condition thread message)
     (assert thread)
     (print (list :debug-condition thread message) *error-output*))
    ((:ping thread tag)
     (slime-send `(:emacs-pong ,thread ,tag) connection))
    ((:reader-error packet condition)
     (print (list :reader-error packet condition) *error-output*)
    ...
    ((:emacs-skipped-packet packet)
     (print (list :emacs-skipped-packet packet) *error-output*))
    (t
